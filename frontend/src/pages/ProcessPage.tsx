...
const TOOL_HINTS: Record<string, string> = {
  'super-resolution': 'Running Real-ESRGAN AI model — CPU processing may take 1–3 minutes.',
  'remove-noise': 'Applying OpenCV denoising…',
  'remove-background': 'Running background removal…',
};
...
export default function ProcessPage() {
  ...
  useEffect(() => {
    ...
    // Animate progress: reaches 99% around 3 minutes so it doesn't pile up early
    const interval = setInterval(() => {
      setProgress((p) => {
        if (p < 50) return p + 4;
        if (p < 80) return p + 0.4;
        if (p < 92) return p + 0.08;
        if (p < 99) return p + 0.015;
        return p;
      });
